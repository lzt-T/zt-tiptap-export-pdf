...
- `src/exportDom.ts`：DOM 解析过渡入口（re-export），对外兼容旧导入路径。
- `src/domParser/`：DOM 解析职责目录，分层处理 ProseMirror 根节点识别、块级节点收集、列表解析、表格读取与块内容提取。
- `src/exportText.ts`：文本样式解析、字号换算、行高解析和 PDF 文本换行。
- `src/exportPdfWriter.ts`：PDF 写入过渡入口（re-export），对外兼容旧导入路径。
- `src/pdfWriter/`：PDF 写入职责目录，分层处理普通文本、引用、代码块与表格写入。
...
- 当单文件接近 500 行且职责混杂时，优先目录化拆分为同域子模块。
